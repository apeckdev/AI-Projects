...
<#
.SYNOPSIS
    Pester tests for the GET /user/login endpoint.
...
Describe "GET /user/login Endpoint Tests" -Tags 'GET', 'user' { # GET, /user/login, 'GET', 'user' are expanded by the *generator*

    Context "Basic GET Request" { # GET expanded by the *generator*

        It "Should return a successful status code (2xx)" {
            # TODO: Handle path parameters (e.g., replace {id} with a valid value) if '/user/login' contains them.
            $requestPath = '/user/login' # Use backtick for literal . /user/login is expanded by the generator.
            ...
            #     Write-Warning "Test for path '/user/login' requires parameter '{$$paramName}'. Using placeholder '1'." # Escape $ for literal  output
