...
<#
.SYNOPSIS
    Pester tests for the POST /user endpoint.
...
Describe "POST /user Endpoint Tests" -Tags 'POST', 'user' { # POST, /user, 'POST', 'user' are expanded by the *generator*

    Context "Basic POST Request" { # POST expanded by the *generator*

        It "Should return a successful status code (2xx)" {
            # TODO: Handle path parameters (e.g., replace {id} with a valid value) if '/user' contains them.
            $requestPath = '/user' # Use backtick for literal . /user is expanded by the generator.
            ...
            #     Write-Warning "Test for path '/user' requires parameter '{$$paramName}'. Using placeholder '1'." # Escape $ for literal  output
