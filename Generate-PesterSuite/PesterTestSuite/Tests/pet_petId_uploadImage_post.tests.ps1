...
<#
.SYNOPSIS
    Pester tests for the POST /pet/{petId}/uploadImage endpoint.
...
Describe "POST /pet/{petId}/uploadImage Endpoint Tests" -Tags 'POST', 'pet' { # POST, /pet/{petId}/uploadImage, 'POST', 'pet' are expanded by the *generator*

    Context "Basic POST Request" { # POST expanded by the *generator*

        It "Should return a successful status code (2xx)" {
            # TODO: Handle path parameters (e.g., replace {id} with a valid value) if '/pet/{petId}/uploadImage' contains them.
            $requestPath = '/pet/{petId}/uploadImage' # Use backtick for literal . /pet/{petId}/uploadImage is expanded by the generator.
            ...
            #     Write-Warning "Test for path '/pet/{petId}/uploadImage' requires parameter '{$$paramName}'. Using placeholder '1'." # Escape $ for literal  output
