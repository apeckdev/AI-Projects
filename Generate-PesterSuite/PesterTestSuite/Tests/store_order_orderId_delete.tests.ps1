...
<#
.SYNOPSIS
    Pester tests for the DELETE /store/order/{orderId} endpoint.
...
Describe "DELETE /store/order/{orderId} Endpoint Tests" -Tags 'DELETE', 'store' { # DELETE, /store/order/{orderId}, 'DELETE', 'store' are expanded by the *generator*

    Context "Basic DELETE Request" { # DELETE expanded by the *generator*

        It "Should return a successful status code (2xx)" {
            # TODO: Handle path parameters (e.g., replace {id} with a valid value) if '/store/order/{orderId}' contains them.
            $requestPath = '/store/order/{orderId}' # Use backtick for literal . /store/order/{orderId} is expanded by the generator.
            ...
            #     Write-Warning "Test for path '/store/order/{orderId}' requires parameter '{$$paramName}'. Using placeholder '1'." # Escape $ for literal  output
            ...
            Write-Verbose "Sending DELETE request to $uri" # DELETE expanded by generator
            ...
            try {
                ...
                # Use backticks for literal , , . 'DELETE' is literal string here.  needs backtick if used.
                $response = Invoke-WebRequest -Uri $uri -Method 'DELETE' -Headers $headers -ErrorAction Stop # Add -Body $body if needed
