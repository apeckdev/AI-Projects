...
<#
.SYNOPSIS
    Pester tests for the DELETE /user/{username} endpoint.
...
Describe "DELETE /user/{username} Endpoint Tests" -Tags 'DELETE', 'user' { # DELETE, /user/{username}, 'DELETE', 'user' are expanded by the *generator*

    Context "Basic DELETE Request" { # DELETE expanded by the *generator*

        It "Should return a successful status code (2xx)" {
            # TODO: Handle path parameters (e.g., replace {id} with a valid value) if '/user/{username}' contains them.
            $requestPath = '/user/{username}' # Use backtick for literal . /user/{username} is expanded by the generator.
            ...
            #     Write-Warning "Test for path '/user/{username}' requires parameter '{$$paramName}'. Using placeholder '1'." # Escape $ for literal  output
