...
<#
.SYNOPSIS
    Pester tests for the DELETE /pet/{petId} endpoint.
...
Describe "DELETE /pet/{petId} Endpoint Tests" -Tags 'DELETE', 'pet' { # DELETE, /pet/{petId}, 'DELETE', 'pet' are expanded by the *generator*

    Context "Basic DELETE Request" { # DELETE expanded by the *generator*

        It "Should return a successful status code (2xx)" {
            # TODO: Handle path parameters (e.g., replace {id} with a valid value) if '/pet/{petId}' contains them.
            $requestPath = '/pet/{petId}' # Use backtick for literal . /pet/{petId} is expanded by the generator.
            ...
            #     Write-Warning "Test for path '/pet/{petId}' requires parameter '{$$paramName}'. Using placeholder '1'." # Escape $ for literal  output
