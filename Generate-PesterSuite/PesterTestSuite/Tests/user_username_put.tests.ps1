...
<#
.SYNOPSIS
    Pester tests for the PUT /user/{username} endpoint.
...
Describe "PUT /user/{username} Endpoint Tests" -Tags 'PUT', 'user' { # PUT, /user/{username}, 'PUT', 'user' are expanded by the *generator*

    Context "Basic PUT Request" { # PUT expanded by the *generator*
        ...
        It "Should return a successful status code (2xx)" {
            ...
            Write-Verbose "Sending PUT request to $uri" # PUT expanded by generator
            ...
            try {
                ...
                # Use backticks for literal , , . 'PUT' is literal string here.  needs backtick if used.
                $response = Invoke-WebRequest -Uri $uri -Method 'PUT' -Headers $headers -ErrorAction Stop # Add -Body $body if needed
