...
<#
.SYNOPSIS
    Pester tests for the GET /user/logout endpoint.
...
Describe "GET /user/logout Endpoint Tests" -Tags 'GET', 'user' { # GET, /user/logout, 'GET', 'user' are expanded by the *generator*

    Context "Basic GET Request" { # GET expanded by the *generator*

        It "Should return a successful status code (2xx)" {
            # TODO: Handle path parameters (e.g., replace {id} with a valid value) if '/user/logout' contains them.
            $requestPath = '/user/logout' # Use backtick for literal . /user/logout is expanded by the generator.
            ...
            #     Write-Warning "Test for path '/user/logout' requires parameter '{$$paramName}'. Using placeholder '1'." # Escape $ for literal  output
